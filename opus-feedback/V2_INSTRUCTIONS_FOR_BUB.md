...
## ⚠️ DAN'S MODIFICATION (implemented)

**Remove follower ceiling entirely (MAX_FOLLOWERS, BIG_BRAND_FOLLOWER_THRESHOLD).**

**Reasoning:** With v2's architecture, brand detection happens via:
1. Domain blocklist (catches known big brands by name)
2. LLM prompt explicitly asks "small/independent or mass production?"
3. Negative keywords catch brand language ("shipping worldwide", "tag us to get featured")

A genuine handmade studio with 200K followers will have "handcrafted", "DM for custom" in bio.
A fast fashion brand at 50K will have "shipping worldwide", "use code", "as seen on".

**The signals are in the language, not the number.**

Keep only MIN_FOLLOWERS = 200 (floor) since accounts <200 lack evaluation presence.
The ceiling just punishes success.
